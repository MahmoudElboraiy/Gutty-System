...
public class PaymobPayment
{
   private readonly string _token;
        ...
        public async Task<PaymobQuickLinkResponse> RequestQuickPaymentLink(string fullName, string email, string phone, int price)
        {
        ...
        private async Task<string> GetToken(string ApiKey)
        {
            ...
            var request = new HttpRequestMessage
            {
                ...
                Method = HttpMethod.Post,
                Content = new StringContent("{\"api_key\":\"" + ApiKey + "\"}", Encoding.UTF8, "application/json")
            };
