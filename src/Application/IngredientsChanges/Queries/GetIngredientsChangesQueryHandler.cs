...
public class GetIngredientsChangesQueryHandler : IRequestHandler<GetIngredientsChangesQuery, ErrorOr<GetIngredientsChangesQueryResponse>>
{
    ...
    public async Task<ErrorOr<GetIngredientsChangesQueryResponse>> Handle(GetIngredientsChangesQuery request, CancellationToken cancellationToken)
    {
        ...
        {
            changesToReturn = changesToReturn.Where(c => c.CreatedAt.Date == request.ChangeDate.Value.Date);
        }
        ...
        return new GetIngredientsChangesQueryResponse(page,pageSize,totalPages,count, hasPreviousPage, hasNextPage, Items);
    }
}