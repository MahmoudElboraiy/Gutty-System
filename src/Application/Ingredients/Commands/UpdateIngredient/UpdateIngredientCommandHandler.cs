...
public class UpdateIngredientCommandHandler : IRequestHandler<UpdateIngredientCommand, ErrorOr<UpdateIngredientCommandResponse>>
{
    private readonly IUnitOfWork _unitOfWork;
    private readonly IIngredientRepository _ingredientRepository;
    private readonly IIngredientLogRepository _ingredientLogRepository;

    public UpdateIngredientCommandHandler(IIngredientLogRepository ingredientLogRepository, IIngredientRepository ingredientRepository, IUnitOfWork unitOfWork)
    {
    ...
    public async Task<ErrorOr<UpdateIngredientCommandResponse>> Handle(UpdateIngredientCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            await _ingredientLogRepository.AddAsync(new IngredientLog()
            {
                IngredientId = oldIngredient.Id,
                Date = DateTime.Now,
                Quantity = Convert.ToInt32(request.Stock) - Convert.ToInt32(oldIngredient.Stock),
                Status = request.Stock > oldIngredient.Stock ? IngredientStatus.Deposit : IngredientStatus.Used
            });
        }
        ...
        return new UpdateIngredientCommandResponse(oldIngredient.Id, oldIngredient.Name, oldIngredient.Stock);
