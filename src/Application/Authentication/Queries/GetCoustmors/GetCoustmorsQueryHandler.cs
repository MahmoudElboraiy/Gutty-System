...
public class GetCoustmorsQueryHandler:IRequestHandler<GetCoustmorsQuery, ErrorOr<GetCoustmorsQueryResponse>>
{
    ...
    public GetCoustmorsQueryHandler(UserManager<User> userManager, IUnitOfWork unitOfWork, ICacheService redisCacheService)
    {
    ...
    {
        string parametersKey = $"page_{request.pageNumber}_size_{request.pageSize}_search_{request.searchName}";

        var response = await _cacheService.GetOrCreateAsync(
           baseKey: CacheKeys.Customers,
           versionKey: CacheKeys.CustomersVersion,
